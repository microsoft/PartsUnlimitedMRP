...
1. You need to change the public SSH key that you will find inside of `~/PartsUnlimitedMRP/Labfiles/AZ-400T05-ImplemntgAppInfra/Labfiles/ansible/new_vm_web.yml` with your own key, which you can find using this command. Copy the key output that is returned after running the command
...
2. Edit the file `/new_vm_web.yml` with a text editor such as **nano** or **vi** from the command line by running the command
    ...
    ```
3. In the nano editor, locate the SSH string towards the end and delete the key that is present in the file.

4. Paste in the key value, it is important that the username matches the user name in your ansibleMaster VM, i.e.`azureuser`, if you followed the lab guide.
...
6.Now we have all the information we need, and we can run the playbook with the required variables. 
...
7. Run the ansible playbook to deploy the virtual  machine by running the below command
...
8. While the playbook is running, have a look in another console inside of the file `/new_vm_web.yml` , and try to identify the different parts it is made out of. 
...
### Task 9: Configure the newly deployed VM with an Ansible playbook
