...
1. Clone the ansible source code from GitHub into your Azure Cloud Shell by running the below command. These source files includes the dynamic inventory files such as `azure\_rm.py`) that we will use for this dynamic inventory task in the lab. Git is already pre-installed with the Azure Cloud hShell
...
3. Install a JSON filter, **jq** that we will use shortly, by running the following command
    ...
    Note: If you receive an error related to permissions, run the following command
...
5. To allow us run python against the azure_rm.py script we need to install the **Azure Python SDK for Ansible**. We cna do that by running the following command
    ```
    sudo pip install ansible[azure]
    ```


5. Test that the dynamic inventory script is running, executing the below command. This should show JSON text containing information about all the VMs in your subscription.
    ...
    Note, if you run the command without the jq json package, the formatting is poor and it is very hard to interpret the output


4. Enable the virtual machine tag by exporting the tag to an environment variable called `AZURE_TAGS` using the belwo comand, you need ot be inthe Bash chell for this command to work.

    ```
    export AZURE_TAGS=nginx
    ``

5. Once the tag has been exported run the Ansible dynamic inventory command against against the resource group and verify only the virtual machine that has the `Nginx` tag assigned is returned.
...
6. You could further refine the inventory script in order to return only the VMs using some other criteria, such as a location, os family, security group membership and more. To achieve that you would modify the *.ini* file that controls some aspects of `azure_rm.py` file. This *.ini* file is located in the same directory as the Python script: `~/ansible/contrib/inventory/azure_rm.ini`. You need to find the line that specifies which resource groups are to be inspected, un-comment it and change it to something like this:
...
7. One final item to be aware of if that you can also run command on the VMs returned by the dynamic inventory script. Run the command below to and verify that data is returned.
