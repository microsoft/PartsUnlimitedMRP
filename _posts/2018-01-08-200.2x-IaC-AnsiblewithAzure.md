...
- Task 6: Configure Credentials and SSH
- Task 7: Run Ansible Playbook to install and configure nginx on our web server
- Task 8: Implement web server desired state scenario
- Task 9: Running Ansible playbooks periodically for configuration management in Azure
- Task 10: Using Ansible with ARM templates for Azure configuration management
- Task 11: Deleting a VM using Ansible - Optional
...
5. Tag the first virtual machine, *ansible-webserver-vm1*,  with the tag `Nginx` to identify it as an **Nginx** web server by running the following command. Don't forge to paste in your subscription ID into the command where indicated. You can obtain the subscription ID by running thr Azure CLI command **az account show**
...
### Task 3: Create a Control VM in Azure using Azure CLI
static inventory is controlled and managed using the /etc/ansible/hosts file. However in the Azure Cloud Shell, we doi not have root access and can only save files in the user context, i.e. `$Home`. As such to perform Ansible management with non dynamic inventory, we need root level access. We can achieve this by running ansible from a full Linux OS. In our case we'll provision a Linux virtual machine to Azure and configur it to allow us manage ansible and Azure virtual machines.
...
9.  Update the *Advanced Packaging Tool* (apt) package list so it has the latest version and package details
    ...
    sudo apt update
    ```
    ![Screenshot of Ubuntu ansible control VM with the apt update command having been run successfully](../assets/feb2019-ansiblewithazure/ansible5.png)
...
12. Verify the Azure CLI is successfully by running the `az` command. If you receive a command not found message, restart the bash shell by running the command below and try again.
...
3. Clone a lab files directory which contains some sample yml files that we will use in the lab by running the command. This repository contains yml for for creating new vms, web servers and more, some of which we will use in the lab.
...
1. Install required software packages
    ...
    Install tje python installed, `pip`, by running the following command
    ```
    sudo apt install python-pip
    ```
   
    Upgrade to the latest version of `pip`, by running the following command
    ```
    ...
    install the json formatting tool, by running the following command
    ```
    sudo apt install jq
    ```
...
### Task 6: Configure Azure Credentials and SSH
Ansible communicates and configures with SSH. As such we need to configure our control virtual machine and hosts, which are being managed, to allow communication between them and with Azure. We will do that in these steps.

1. You will also need to create a new file in the directory `~/.azure` (create it if it does not exist), using the credentials generated in the previous sections. The filename is `~/.azure/credentials`.
...
2.  And lastly, we will create a public/private key pair, and install the public key in the local machine, to test the correct operation of Ansible. Press **Enter** when prompted, to accept the default values for the file in which to save the key, and to leave the passphrase (password) empty.
...
3.     When completed, then run the below commands to set the file and folder permissions.
...
4. Next run the below command and when prompted enter the password for the **azureuser** which you defined earlier i.e. Password0134! or whatever you set it as.
...
5. You can now verify that when trying to `ssh` to the local machine, no password will be requested when running the below command:
    ...
    ```
6. Type exit once only, to exit from the 127.0.0.1 connection
...
1. In Azure Cloud Shell clone a git repo with some playbook files, one of which we will use in this section. clone the PartsUnlimitedMRP git repo into your Azure Cloud Shell instance by running the command
...
2. We dynamic inventory is set to filter for the tag `nginx` which applies to `ansible-webserver-vm1` will now run a playbook against that virtual machine to ensure nginx is installed and the latest version by running the following command


    ```
    ansible-playbook -i ~/ansible/contrib/inventory/azure_rm.py ~/PartsUnlimitedMRP/Labfiles/AZ-400T05-ImplemntgAppInfra/Labfiles/ansible/nginx.yml --extra-vars  "vmname=ubuntuwebvm1"
    ```
