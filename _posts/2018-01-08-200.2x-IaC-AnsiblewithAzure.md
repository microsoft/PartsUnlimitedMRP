...
5. Create a Linux virtual machine in that subnet with a public IP address.
    ...
    ```
    az vm create -n ansibleMaster --image OpenLogic:CentOS:7.6:latest --vnet-name ansibleVnet --subnet ansibleSubnet --public-ip-address masterPip --authentication-type password --admin-username azureuser --admin-password Password0134!
    ```
...
7.  Install Azure CLI 2.0 in the virtual machine you have just created and connected to, the provisioning machine `ansibleMaster`, by running the below commands in succession. The first command listed below, may take up to 10 minutes to complete.
    ...
    - **Note:** You can just press **Enter** to accept the default answers when prompted by the installation. However, in the third question prompt, make sure to tell the script to update the PATH variable (answer **Y**). There may also be some errors or red text output, for incompatability or deprecation, it is fine to ignore these once the installation completes in successfully in full. 
