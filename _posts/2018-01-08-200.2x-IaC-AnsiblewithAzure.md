...
10. Run the following commands in sequence to install some pre-requisite packages that are required for the Azure CLI, which we will install shortly into our ansible control VM.
    ```
...
11. Install the Azure CLI by running the below command
    ...
    You can just press **Enter** to accept the default answers when prompted by the installation. However, when prompted to update the `PATH` variable, make sure to answer **Y**. There may also be some errors or *red* text output, for incompatibility or deprecation, it is fine to ignore these for the purposes of our lab, once the installation completes successfully. 
...
12. Verify the Azure CLI is successfully by running the `az` command. If you receive a command not found message, restart the bash shell by running the command below and try again.
    ...
    ![Screenshot of command prompt with az command returning a command not found error then, restarting the shell and the az command running successfully ](../assets/feb2019-ansiblewithazure/ansible43.png)
