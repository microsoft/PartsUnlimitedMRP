...
4. You will also need to create a new file in the directory `~/.azure` (create it if it does not exist), using the credentials generated in the previous sections. The filename is `~/.azure/credentials`.
    ...
    Run the first line below for the cat command, and then one line at time enter each line and press return, replacing the numbers listed below with the actual values you obtained for AppID, SubscriptionID and tenant id. When you paste in the EOF value at the end it will save and exit.
