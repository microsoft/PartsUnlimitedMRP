...
10. Run the following commands in sequence to install some pre-requisite packages that are required for the **Azure CLI**, **Azure Python SDK** and **Ansible**, which we will install shortly into our ansible control VM.
...
1. Install required software packages
    ...
    Install the python package manager, `pip`, by running the following command
    ```
...
2. Install **Azure Python SDK for Ansible**. Additionally, we will install the package `DNS Python` so that we would be able to do DNS checks in Ansible playbooks (to make sure that DNS names are not already taken). Some Python versions also require the `packaging` module to be installed separately.
...
Ansible communicates and configures using `SSH`. As such we need to configure our control virtual machine and hosts, which are being managed, to allow communication between them and with Azure. We will do that in these steps.

1. You will also need to create a new file in the directory `~/.azure` (create it if it does not exist), using the credentials generated in the previous sections. The filename is `~/.azure/credentials`. Start by running the below commands in sequence.
