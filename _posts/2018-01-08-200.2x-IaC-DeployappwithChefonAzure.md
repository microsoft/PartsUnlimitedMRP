...
5. You can obtain the VMID a number of ways, i.e. using PowerShell, or Azure CLI, but we will connect to the chef automate VM using PuTTy and run the command provided in the screenshot in step 4 above.
	...
	**Obtain VMID using PuTTy**
	...
	- Obtain PuTTy, from <a href="https://putty.org/" target="_blank"><span style="color: #0066cc;" color="#0066cc">https://putty.org/</span></a>, and open a connection to the deployed **Chef Automate VM** 
		...
		in a Linux environment the connection could be done using ssh.
...
The details you enter here, are entered into the Chef Automate VM deployment. Whatever details you specify you need to note for later use, but for ease of use we would recommend to use the following and use them through the lab for use with the Chef Automate server, git and the Linux VM deployment.
