...
7. In the starter kit the extracted files location, open the file `chef-repo\.chef\knife.rb` file and check that the `chef_server_url` to the external FQDN is as below (e.g. **https://<chef-server-dns-name>.<region>.cloudapp.azure.com/organizations/default**). Then, save and close the file.
