...
7. In the starter kit the extracted files location, open the file `chef-repo\.chef\knife.rb` file and check that the `chef_server_url` to the external FQDN is as below ( e.g. **https://<chef-server-dns-name>.<region>.cloudapp.azure.com/organizations/default** ). Then, save if need be, and close the file.
...
8. In the **ChefDK shell** window, in which you ran `chef verify` earlier, Change directories to the `chef-repo` directory  (i.e. `cd C:\Users\<username>\chef\chef-repo`). 
