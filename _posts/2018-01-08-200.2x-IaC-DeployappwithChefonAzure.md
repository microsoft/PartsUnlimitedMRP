...
In this hands-on lab, you will explore some of the features and capabilities of deploying the PartsUnlimted MRP Application via Chef Server in Azure. This hands-on lab is designed to point out features, discuss and describe them, and enable you to understand and explain these features to customers as part of the DevOps Lifecycle.
...
<iframe width="960" height="640" src="https://lexvideoassets.blob.core.windows.net/finalfiles/DEVOPS200.1/devops on azure paas/17082018-DEVOPS200.2-IaC-DeployAppwuthChefonAzure.mp4?sv=2015-04-05&sr=b&sig=a6Nyw0Qo4umqTlI3WSJrJa18zohNtLyNlEatOmgt2tI%3D&se=2018-10-31T19%3A21%3A15Z&sp=r" frameborder="0" allowfullscreen></iframe>
</div>
</div>
...
1. This lab calls for the use 3 different environments i.e. 2 virtual machines on Azure, and your local environment. You could if you wished also create a VM to use in lieu of your local environment if you wished. The thre environments are as follows:
	- The Chef Automate which houses the chef server must be a Linux machine. 
	- The MRP app client will be a Linux machine. This is the Chef node machine which will be configured and have the MRP application deployed to it by Chef.
	- The Chef workstation from where you will connect to and manage your Chef server from. It can be Linux, Windows, or Mac. 
...
2. Click the **Deploy to Azure** button below and follow the wizard to deploy the Chef Automate machine. You will need to log in to the Azure Portal.
...
3. You will need to add details for the following: 
	...
	**Note:** In some instances a unique name is required. Where a unique name is required, you can add your initials or some other unique identifier to it to ensure this is the case.

	- Subscription: **< your Azure subscription >** 
	- Resource group:  **< a unique resource group name i.e. chefrgek017 >**
	- Location: **< select a region to deploy the Resource Group to i.e. West Europe, East US, etc >**
	- Admin Username: **azureuser** 
	- Admin Password: **Passw0rd0134**
	- Diagnostic Storage Account Name: **< a unique name i.e. **diagstrek017** >**
	- Location: **< select a region to deploy the VM to. Use the same as earlier i.e. West Europe, East US or whatever you selected earlier >**
	- Public IP Dns Name: **< a unique dns name, i.e. **chefautomateek017**>**
	- Storage Account Name: **< a unique name i.e. **strek017** >**


	**Note:** Make sure not to change any other configuration fields as they are populated correctly for you, just edit the ones called out above and in the screenshot below. Also, make sure you take a note of the region as well as the usernames and passwords for the machine. 
...
5.  Check that Chef Automate deployment was successful by going to the resource group then **Overview** and you can view the **Deployments**
	...
	You can view the detalis of the deplpoyments by clicking **Deployments** 
	![](../assets/pumrpdeploywithchef-jan2018/chef_deployment2.png)	


6. Obtain the **Public IP Address** by selecting the **chefautomateip** resource (or whatever you called it) and copy the **DNS name**.
...
7. Open a browser and go to the public DNS name i.e. to **https://_dnsaddress_**. (Make sure you're going to *http`s`*, and **not** *http*). You will be prompted about an invalid certificate, it is safe to ignore this for the purposes of this lab. Your prompts and steps to access the page may be different depending on your browser. The below screenshot is using *Edge*.
...
In this exercise, you will configure your Chef Workstation by setting up the Chef Starter Kit on your local workstation. It is from the Chef workstation that you will connect to and manage your Chef server.

1. Download and install the **Chef Development Kit** from <a href="https://downloads.chef.io/chefdk" target="_blank"><span style="color: #0066cc;" color="#0066cc">https://downloads.chef.io/chefdk</span></a>, choosing the appropriate version for your environment, i.e. **Mac OSX/macOS**, **Debian**, **Red Hat Enterprise Linux** **SUSE LInux Enterprise Server**,**Ubuntu**, **Windows** and choosing the correct flavour within those groupings.

	**Note**: As of Aug 2018 the latest version of the Chef sdk is 3.1.0 This is the version that was used in these lab steps.

2. Once installed, open the **Chef Development Kit** shell, you should have a desktop shortcut for **Chef Development Kit** on your desktop, double click this shortcut, and run the command 
	...
	**Running `Chef Verify`**
	...
	The `chef verify` command returned errors that git was not configured with your identity information. Proceed with step 3 to configure your identify information in git.

3. In the **ChefDK** window Configure your global git variables with your name and email address if you haven't already. We will use git to store  our chef configuration details.
...
4. Now you will need to get the **Chef Starter Kit** for accessing **Chef Automate**. To get to the **Starter Kit** you will need to access the **Chef Automate VM** that you installed earlier in Task 1, with the following path **https://<dns_label>.<location>.cloudapp.azure.com/biscotti/setup**

	![](../assets/pumrpdeploywithchef-jan2018/shot6.png)

	You are prompted to enter the **VMID** as per the screenshot above. This is the virtual machine ID for the Chef Automate server VM that we deployed Task 1.

5. The **VMID** is a unique value each VM in Azure is assigned. You can obtain the VMID a number of ways, i.e. using PowerShell, or Azure CLI, but we will connect to the Chef Automate VM using PuTTy and run the command provided in the screenshot in step 4 above. Likewise in a Linux environment you can connect using ssh to establish a connection to the Chef Automate VM.

	**Obtain VMID using PuTTy**
	
	- Obtain PuTTy, from <a href="https://putty.org/" target="_blank"><span style="color: #0066cc;" color="#0066cc">https://putty.org/</span></a>, and open a connection to the deployed **Chef Automate VM** 

		![](../assets/pumrpdeploywithchef-jan2018/Chef_automateputtyconnect1.png)

	
		
	- Login into the Chef Automate VM using the details you specified for user name and password i.e. 
		- username=**azureuser**
		- password=**Passw0rd0134**


	- Once logged into the Chef Automate VM, run the following command and note the VMID that is returned

	```
	sudo chef-marketplace-ctl show-instance-id
	```

	![](../assets/pumrpdeploywithchef-jan2018/getvmid1.png)


	- Enter the VMID into the **Chef Automate** page and click **Authorize**, as in the screenahot below

	![](../assets/pumrpdeploywithchef-jan2018/vmid2.png)


6. The details you enter here, are entered into the Chef Automate VM deployment. Whatever details you specify you need to note for later use, but for ease of use we would recommend to use the following, as we have already done, and use them throughout the lab for use with the Chef Automate server, git and the Linux VM deployment.
	...
	![](../assets/pumrpdeploywithchef-jan2018/chefautomatesetup1.png)

	Note these values down as you will need the values for username, email address, etc later.

	**NOTE:** An organization name of **default** will be used, we do not speficy an organization name in this setup. You can do so within the Chef Automate server but we will just use the default value, and this is the organization value that will appear in the `knife.rb` file later in Task 3. 

7. Agree to the EULA terms and click **Set Up Chef Automate**


	![](../assets/pumrpdeploywithchef-jan2018/chefautomatesetup2.png)

	Again you will get a warning message about the certificate, but continue to the web page.



8. You are taken to a page where you can download the **Starter Kit**. It is a file named **starter-kit.zip** which contains some pre-configured files to get us going quickly. It should be downloaded to a local folder i.e. **C:\Chef**. Click **Download Starter Kit** to download the zip file.

	**Note**: The starter kit contains credential, user details as well as certificates. If you register on different occasions the details generated each time will be specific to the user and organization details you specified at that time. So you should not re-use the start kit files across multiple users or registrations as you may encounter errors which you'd bneed to track down.

	![](../assets/pumrpdeploywithchef-jan2018/shot7.png)


9. After you have downloaded the **Chef Starter Kit**, click **Login to Chef Automate** and enter the user name and password you just provided in the **Chef Automate Azure Marketplace Setup** page and sign into Chef Automate. The **Chef Automate dashboard** should be visible

	![](../assets/pumrpdeploywithchef-jan2018/Chef_chefautomatedashboard.png)

10. Extract the Chef starter kit files to a directory like `C:\Users\<username>\chef\`.

11. In the starter kit the extracted files location, open the file `chef-repo\.chef\knife.rb` file and check that the `chef_server_url` to the external FQDN is as below ( e.g. **https://<chef-server-dns-name>.<region>.cloudapp.azure.com/organizations/default** ). Then, save if need be, and close the file.

	**Note**: If you have a different organization name present in the knife.rb file change it to the default organization.

	![](../assets/pumrpdeploywithchef-jan2018/change_chef_url.png)

12. In the **ChefDK shell** window, in which you ran `chef verify` earlier, Change directories to the `chef-repo` directory  (i.e. `cd C:\Users\<username>\chef\chef-repo`). 
...
14. View the current **chef-repo** contents, by running either **ls** or **dir**. This is just to allow you to be more aware of files that will be added shortly versus what is currently present.
...
15. Synchronize the Chef repo by running the below command, this downloads the entire chef repo from your Chef Automate server, see more specific details <a href="https://docs.chef.io/knife_download.html" target="_blank"><span style="color: #0066cc;" color="#0066cc">here</span></a>
	...
	You may see errors in relation to "acls" depending on your user account details, but it is fine to ignore these for the purposes of the remainder of this lab.

16. Run the **dir** command again, and observe that additional files and folders have been created in the **chef-repo** directory. 
...
17. Commit the added files into the git repository:
...
In this exercise, you will create a cookbook and recipe for the PartsUnlimited MRP app's dependencies to automate the installation of the MRP application, and upload those cookbooks and recipes to the Chef server. 
...
1. Open the **Chef Development kit** window and go to the folder **cookbooks** within the **chef-repo**. i.e. `cd C:\Users\eamonnk\chef\chef-repo\cookbooks` or wherever you placed your git repo in Task 2. Then use the **knife** tool to generate a cookbook template by running the following command
	...
	A cookbook is a set of tasks for configuring an application or feature. It defines a scenario and everything required to support that scenario. Within a cookbook, there are a series of recipes that define a set of actions to perform. Cookbooks and recipes are written in the Ruby language. The command we run here creates an **mrpapp** directory in the `chef-repo/cookbooks/` directory that contains all of the boilerplate code that defines a cookbook and a default recipe.

	![](../assets/pumrpdeploywithchef-jan2018/view_mrp_cookbook.png)

2. Edit the `metadata.rb` file in our **cookbook/mrpapp** directory i.e. open `chef-repo/cookbooks/mrpapp/metadata.rb` for edit. Cookbooks and recipes can leverage other cookbooks and recipes. Our cookbook will use a pre-existing recipe for managing APT repositories. Add the following line at the end of the file, then save and close the file.
...
4. The last command may change the focus in the **Chefdk** window to the **chef-vendor-windows** branch of your repo. Switch back to the **master** branch (this should happen automatically when you run the command but may possibly fail, and as such you need to change it back to **master** manually if it does).
...
5.  Copy the full contents of the recipe from <a href="https://raw.githubusercontent.com/Microsoft/PartsUnlimitedMRP/master/Labfiles/DevOps200.2x-InfrastructureasCode/DeployusingChef/final/default.rb" target="_blank"><span style="color: #0066cc;" color="#0066cc">https://raw.githubusercontent.com/Microsoft/PartsUnlimitedMRP/master/Labfiles/DevOps200.2x-InfrastructureasCode/DeployusingChef/final/default.rb</span></a>
...
8. **The following explains what the recipe is doing to provision the application.**
	...
	Next, we will use the **apt-package** recipe to ensure that the **OpenJDK** and **OpenJRE** are installed. We use the **headless** version as the full version has a dependancy on a legacy package we we don't want to use. 
...
10. Now that we have a recipe created and all of the dependencies installed, we can upload our cookbooks and recipes to the **Chef Automate server** with the `knife cookbook upload` command. From the command line, run: 
...
2. Fill in with the following details. 
	...
	- Location: **< whatever location you selected for the Chef Automate i.e. West Europe >**
	- adminUsername: **azureuser**
	- adminPassword: **< your admin password i.e. Passw0rd0134 >**
	- dnsLabelPrefix: **appmrppartsek017**
...
3. Once this is deployed, return to the **Chefdk** windows and use knife to boostrap the VM by running the following command: 
	...
	Note: The **FQDN-for-MRP-App-VM** is the MRP application VM you just deployed in the previous step.
	...
	The script should take between 5 to 10 minutes to run. You will see it do the following things:
	...
	**Potential Errors and Issues**
	...
	- If there is an error related to resolving the run list you will need to run the below command and try again. This will make sure the missing role is in the node
...
6. Log into the Chef Automate server and go to **Nodes**. Verify the virtual machine is listed as a node and click through various details related to **Resources**, **Run List** and **Attributes**, verify the details are as expected.
	...
	Note also, that you can manage your Chef server configuration and node deployments, via a browser by installing the Chef Management Console. Go to your Chef Automate server deployment organization page i.e. similar to the following url, **https://< your chef automate server >.< eastus >.cloudapp.azure.com/organizations/default** and see the details about accessing and managing and your Chef server.

	![](../assets/pumrpdeploywithchef-jan2018/Chefsrvmgmtconsole1.png)
...
- Bootstrapped the linux virtual machine to add the role to the node i.e. deployed PartsUnlimitedMRP application to it using Chef.
