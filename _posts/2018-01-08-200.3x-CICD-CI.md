...
**22.** In the repo go to the location **/src/Clients/Web/pages/main/main.html** select **Edit** and modify line 17 from 
...
- **deploy**: scripts and templates to configure and deploy the app to our app server
- **drop**: one containing the actual application to be deployed. 
...
We will use these build artifacts in our subsequent condintuous deployment lab, where we will deploy our application, thus extending out build and release pipeline. You can download and have a look through these biuld artifacts if you have time.
