...
In this hands-on lab you deployed the PartsUnlimited MRP App via Chef Automate server in Azure to an applicatin server. We completed the following tasks. 
...
- Created a cookbook and recipe for the PU MRP application's dependencies, to automate the installation of the MRP application. Then uploaded that cookbook to the Chef Automate server.
...
- Bootstrap the linux virtual machine to add the role to the node i.e. deployed PartsUnlimtedMRP application to it using Chef.

This hands-on lab was designed to point out new features, discuss and describe them, and enable you to understand and explain these features as part of the DevOps Lifecycle.
