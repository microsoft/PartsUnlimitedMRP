...
2. Install additional **Puppet Forge Modules:** We will now install some modules from the Puppet Forge that we will need to configure the `partsmrp` node. Run the following 3 commands in the open ssh terminalk connection to the puppet master:
