...
- Line 3: We require the `Tomcat` user accoutn and group to be available before we can configure tomcat, so we make the class createuserandgroup as required.
- Line 6-9: We install Tomcat from the Tomcat module we downloaded earlier, the package name is `Tomcat7`, so we call it here, we designate the home directory where we will place our default installation and we specify that it is not fro source, this is required when installing from a downloaded package. We could install directly from a web source also if we wished.
- Lines 13 - 18: We configure the Tomcat connector, define the protocol to use, the port number, ensuring the connector is present to allow it serve requests on these settings. we also specify the connector properties for Puppet to write to the tomcat server.xml file.
- Lines 22 - 25: We configure the Tomcat service, whic is our default installation, as there is only one installation of Tomcat we specify the default value that we're applying the config to, ensure the service is runing and the start the service using the `use_init` value, we then specify a name for the service i.e. **tomcat7*,  connector for the Parts Unlimited MRP application. 
