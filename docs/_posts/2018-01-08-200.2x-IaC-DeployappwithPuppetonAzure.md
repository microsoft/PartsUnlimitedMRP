...
> All thats typically needed to deploy a `.war` file is to copy the `.war` file to the tomcat `/var/lib/tomcat7//webapps` folder, then once the correct permissions are in place on the folder and the values for `unpackWARS` and `autoDeploy` in the tomcat server config file `/var/lib/tomcat7/conf/server.xml`. Tomcat will extract and then deploy the .war.
