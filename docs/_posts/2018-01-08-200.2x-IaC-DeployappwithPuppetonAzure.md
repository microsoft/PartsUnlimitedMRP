...
2. Install additional **Puppet Forge Modules:** 
    ...
    >**Note:** The `mongodb` and `tomcat` modules are supported modules from the Forge. The `wget` module is a user module and so is not officially supported. The account module will provide us with puppet classes to manage and create user and groups on the linux machine.
